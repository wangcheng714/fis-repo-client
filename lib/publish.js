...
module.exports = function(dir, options, callback){
    ...
    dir = self.util.realpath(dir);

    if(self.util.exists(pkgFile)){
        ...
        if(username && auth){
            ...
            self.login.hasAuth(user, pkg, op, options, function(error, message){
                if(error){
                    ...
                    //todo gz和tar.gz 都有问题，暂时只能使用zip，后续有时间了在看看具体原因
                    ...
                    archive.finalize(function(error, written){
                        if(!error){
                            ...
                            var req = request.post(url, function(error, res, body){
                                if(self.util.isFile(pkgTgz)){
                                    self.util.del(pkgTgz);
                                }
                                if(error){
                                    callback(error);
                                }else{
        ...
        var errorMsg = "Could not publish， missing file [" + dir + "/package.json]";
        callback(errorMsg);
    }
};
